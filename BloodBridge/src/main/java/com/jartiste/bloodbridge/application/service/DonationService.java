...
public class DonationService {
    ...
    public AssociationResult associateDonorToReceiver(Long donorId, Long receiverId) {

        if (null == donorId || null == receiverId) {
            logger.error("Donor ID and Receiver ID cannot be null");
            return new AssociationResult(false, "Donor ID and Receiver ID cannot be null");
        }
        ...
        try {
            ...
            if (donorOpt.isEmpty()) {
                ...
                return new AssociationResult(false, "Donor with ID " + donorId + " does not exist");
            }
            ...
            if (donationRepository.hasDonorGivenToReceiver(donorId, receiverId)) {
                ...
                return new AssociationResult(false, "Donor with ID " + donorId + " has already given a bag to receiver ID " + receiverId);
            }
            ...
            if (receiverOpt.isPresent()) {
                ...
                if (receiver.getReceivedBags() >= receiver.getRequiredBags()) {
                    ...
                    return new AssociationResult(false, "Receiver with ID " + receiverId + " has already received the necessary bags");
                }
            }

            if (receiverOpt.isEmpty()) {
                logger.error("Receiver with ID {} does not exist", receiverId);
                return new AssociationResult(false, "Receiver with ID " + receiverId + " does not exist");
            }
            ...
            if (donor.getStatusDonneur() != StatusDonner.AVAILABLE) {
                ...
                return new AssociationResult(false, "Donor with ID " + donorId + " is not available for donation");
            }
            ...
            if (null != donor.getDernierDon() && donor.getDernierDon().
                ...
                return new AssociationResult(false, "Donor with ID " + donorId + " has donated in the last month");
            }
            ...
            if (!donor.getBloodType().isCompatibleWith(receiver.getBloodType())) {
                ...
                return new AssociationResult(false, "Blood type of donor ID " + donorId + " is not compatible with receiver ID " + receiverId);
            }
            ...
            if (!associated) {
                ...
                return new AssociationResult(false, "Failed to associate donor ID " + donorId + " with receiver ID " + receiverId);
            }

            return new AssociationResult(true, "Donor ID " + donorId + " successfully associated with receiver ID " + receiverId);
        } catch (Exception e) {
            logger.error("Error associating donor to receiver: ", e);
            return new AssociationResult(false, "Error associating donor to receiver: " + e.getMessage());
        }
    }
}
